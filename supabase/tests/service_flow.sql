...
begin
  ...
  -- AT-20: DP 50.000 sebelum total ditentukan -> UNPRICED (bukan lunas).
  ...
  perform pg_temp.check(v_pay->>'purpose' = 'DEPOSIT' and v_pay->>'change' = '50000', 'DP tunai + kembalian server');
  perform pg_temp.check(v_pay->'payment'->>'status' = 'UNPRICED', 'DP -> UNPRICED');
  perform pg_temp.check(pg_temp.cash_expected('SHOP_DRAWER') = 250000, 'DP tunai masuk laci');

  -- Konvensi: command = nama RPC, dapat dicek dengan get_operation_v1; replay tidak menggandakan.
  ...
  perform pg_temp.check(v->>'total' = '150000' and v->'payment'->>'status' = 'PARTIAL'
    and v->'payment'->>'outstanding' = '100000', 'final 150.000 sisa 100.000');
  ...
  -- Pelunasan kurang/lebih ditolak; tepat sisa diterima (TRANSFER wajib konfirmasi).
  ...
  perform pg_temp.fail('record_service_payment_v1', jsonb_build_object('ticket_id', v_ticket, 'amount', '100000',
    'method', 'TRANSFER'), 'CONFIRMATION_REQUIRED');
  v := pg_temp.call('record_service_payment_v1', jsonb_build_object('ticket_id', v_ticket, 'amount', '100000',
  ...
  perform pg_temp.check(v->>'custody_location' = 'CUSTOMER' and v->>'closed_at' is not null, 'serah terima menutup');
...
begin
  ...
  v := pg_temp.call('record_service_payment_v1', jsonb_build_object('ticket_id', v_onsite, 'amount', '100000',
    'method', 'CASH', 'cashbox', 'FATHER_WALLET', 'tendered', '100000'));
  perform pg_temp.check(pg_temp.cash_expected('FATHER_WALLET') = 100000 and pg_temp.cash_expected('SHOP_DRAWER') = 250000,
  ...
  perform pg_temp.check((select closed_at is not null and custody_location = 'CUSTOMER' from private.service_tickets where id = v_onsite)
