-- P2 RPC: finalize_sale_v1 — finalisasi penjualan barang atomik (BR-04 exact)
...
begin
  ...
  -- PASS 1: validasi + hitung base_net per baris
  for v_line in select value from jsonb_array_elements(p_input->'items') loop
    ...
    if not found then raise exception 'Satuan tidak ditemukan atau tidak aktif' using errcode='22023'; end if;
  ...
  -- Alokasi diskon nota: floor(D*B_i/S), sisa ke remainder terbesar, seri line_no naik
  v_alloc_arr := '{}';
  v_rem_arr := '{}';
  v_floor_sum := 0;
  ...
  -- PASS 2: invoice_items + stok + cost allocation
  v_line_no := 0;
  for v_line in select value from jsonb_array_elements(p_input->'items') loop
    ...
    -- Kurangi stok + cost allocation (FIFO by posted_at, lot id, position id)
    for v_pos in select * from private.stock_positions s
        join private.inventory_lots l on l.id = s.lot_id
        where l.product_id = v_product.id
        and s.location = 'SHOP' and s.condition = 'SALEABLE' and s.qty_base > 0
      ...
      if v_pos.qty_base >= v_qty_base then
        ...
        -- Ledger: catat movement penjualan
        ...
        -- Ledger: catat movement penjualan (partial)
        insert into private.stock_movements(group_id, lot_id, position_id, qty_delta, cost_delta,
          kind, invoice_item_id, actor_id, operation_id)
  ...
  if v_total = 0 then
    -- Gratis oleh owner hanya lewat diskon 100%; tidak ada payment
    null;
